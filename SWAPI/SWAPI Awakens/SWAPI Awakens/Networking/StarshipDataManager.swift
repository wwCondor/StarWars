...
extension Client {
    ...
    static func findShortestAndLongestStarship(starships: [Starship]) -> (Starship?, Starship?) {
        ...
        let filterCommaStarships = filteredStarships.filter { !comma.contains($0.length!) }
        ...
        let sortedStarships = filterCommaStarships.sorted(by: { if let length1 = Double($0.length!), let length2 = Double($1.length!) {
